...
const Medias = () => {
    ...
    return (
        <div className="min-h-screen bg-[#06021b] flex font-sans selection:bg-indigo-500/30 overflow-x-hidden">
            {message.active && (
                <div className={`fixed top-6 right-6 z-[100] flex items-center gap-4 px-6 py-5 rounded-2xl shadow-2xl border backdrop-blur-3xl ${message.type === 'success' ? 'bg-indigo-500/10 border-indigo-500/50 text-indigo-400' : 'bg-rose-500/10 border-rose-500/50 text-rose-400'
                    }`}>
            ...
            <main className="flex-1 lg:ml-80 p-6 md:p-12 transition-all">
                <div className="max-w-4xl mx-auto">
                    ...
                    <form className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden">
                        <div className="space-y-8">
                            <div className="group relative">
                                ...
                                <div className="relative min-h-[160px] border-2 border-dashed border-white/10 rounded-3xl flex flex-col items-center justify-center gap-4 hover:border-indigo-500/50 hover:bg-white/5 transition-all cursor-pointer p-4">
                            ...
                            <div className="space-y-8">
                                <div className="space-y-3">
                                    ...
                                    <input type="text" name="name" onChange={handleChange} value={data.name} placeholder="e.g. Vacation Video" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white placeholder:text-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/40" />
                                </div>
                                <div className="space-y-3">
                                    <label className="text-gray-500 text-[10px] font-black uppercase tracking-[0.25em] ml-2">Description</label>
                                    <textarea name="desc" value={data.desc} onChange={handleChange} placeholder="Optional notes..." className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white h-[100px] resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500/40" />
                                </div>
                            </div>

                            <button onClick={upload} disabled={isUploading} className={`w-full md:w-max px-14 py-5 bg-indigo-600 text-white font-black uppercase tracking-[0.3em] text-[11px] rounded-2xl transition-all shadow-2xl flex items-center justify-center gap-3 ${isUploading ? 'opacity-70' : 'hover:bg-indigo-500 active:scale-95'}`}>
                    ...
                    <div
                        className="mt-8 bg-gradient-to-r from-indigo-950/30 to-black border border-white/5 rounded-[2.5rem] p-8 flex flex-col md:flex-row justify-between items-center gap-6 group hover:border-indigo-500/40 transition-all cursor-pointer"
